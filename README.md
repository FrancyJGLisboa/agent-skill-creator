...
**Claude Code — native plugin (no shell, in-tool updates):**
...
Skills generated by the factory ship the same `.claude-plugin/` manifests, so any skill it builds is also installable with `/plugin marketplace add <repo-or-local-path>`.
