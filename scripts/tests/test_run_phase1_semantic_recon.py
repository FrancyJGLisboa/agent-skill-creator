...
from run_phase1_semantic_recon import run  # noqa: E402
...
def test_phase_one_wrapper_emits_scope_and_skips_non_required_recon(tmp_path):
    ...
    (skill / "discovery.json").write_text(
        json.dumps({"data_interfaces": {"applies": False}})
    )
...
def test_phase_one_wrapper_requires_discovery(tmp_path):
    assert run(tmp_path) == {
        "status": "BLOCKED_NO_DISCOVERY",
        "detail": str(tmp_path / "discovery.json"),
    }
