...
def derive(discovery: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    if not isinstance(kinds, list):
        return []
    triggers = " ".join(
        str(x).lower() for x in discovery.get("trigger", []) if isinstance(x, str)
    )
    reuse = (
        "recurring"
        if any(
    ...
    impact = (
        str(risk.get("tier", "low")).lower() if isinstance(risk, Mapping) else "low"
    )
    sources = interfaces.get("authoritative_sources", [])
    labels = [str(x) for x in sources if isinstance(x, str)] or [
        str(kind) for kind in kinds
    ]
    return [
        {"id": slug(label), "type": str(kind), "reuse": reuse, "impact": impact}
...
def emit(path: Path) -> dict[str, Any]:
    ...
    recon = data.get("semantic_recon")
    if not isinstance(recon, dict):
        recon = {"applies": bool(derive(data))}
    derived = derive(data)
    ...
    if not derived and existing_by_id:
        ...
        recon["sources"] = [
            {**existing_by_id.get(source["id"], {}), **source} for source in derived
        ]
...
if __name__ == "__main__":
    ...
    result = emit(Path(args.discovery_json))
    print(json.dumps(result["semantic_recon"], ensure_ascii=False))
