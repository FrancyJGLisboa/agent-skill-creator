...
def main() -> int:
    ...
    p.add_argument(
        "--timeout-seconds",
        type=int,
        default=int(os.environ.get("SEMANTIC_RECON_TIMEOUT_SECONDS", "600")),
    )
    args = p.parse_args()
    source = Path(args.source_json).read_text(encoding="utf-8")
    out = Path(args.result_json).resolve()
    contracts = Path.home() / "contracts"
    sandbox = os.environ.get("SEMANTIC_RECON_SANDBOX", "workspace-write")
    if sandbox not in {"workspace-write", "danger-full-access"}:
        print(
            "SEMANTIC_RECON_SANDBOX must be workspace-write or danger-full-access",
            file=sys.stderr,
        )
        return 2
    if not shutil.which("codex"):
        print("codex CLI is not installed", file=sys.stderr)
        return 2
    ...
    try:
        ...
        write_failure(
            out,
            f"runner produced no valid result JSON: {exc}; agent transcript: {detail}",
        )
